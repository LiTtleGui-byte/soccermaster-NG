...
from .video_utils_siglip import read_frames_decord
...
from ..paths import LLAMA_ROOT, SIGLIP2_ROOT
...
class MatchVisionCommentary_new_benchmark_from_npy_Dataset(Dataset):
    def __init__(self, json_file, video_base_dir, 
                 ...
                 tokenizer_name = str(LLAMA_ROOT), max_token_length =128,
        ...
        self.transform = set_transform(visual_encoder_model_name)
    ...
    def collater(self, instances):
        input_ids = [
            torch.cat((torch.tensor([self.tokenizer.convert_tokens_to_ids("<|begin_of_text|>")]),
                       ...
                       torch.tensor([self.tokenizer.convert_tokens_to_ids("<|end_of_text|>")]))).to(torch.long) for instance in instances] # add end token
