...
DEFAULT_MANIFEST = REPO / "reproduction/manifests/g10_prerefiner_enrichment_run2_sngs10004.json"
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if identity != (1, "G10-B", "prerefiner_enrichment_run2_prepared"):
...
def validate_run2_delta(manifest: dict[str, Any]) -> dict[str, Any]:
    references = manifest["references"]
    run1_manifest = json.loads(Path(references["run1_manifest"]["path"]).read_text(encoding="utf-8"))
    run1_config = yaml.safe_load(Path(references["run1_config"]["path"]).read_text(encoding="utf-8"))
    run2_config = yaml.safe_load(Path(manifest["config"]["path"]).read_text(encoding="utf-8"))
    run1_result = json.loads(Path(references["run1_result"]["path"]).read_text(encoding="utf-8"))
    diagnosis = json.loads(
        Path(references["dataset_diagnosis_run3_result"]["path"]).read_text(encoding="utf-8")
    )

    expected_run1_failure = {
        "outcome": "failed",
    ...
    for key, expected in expected_run1_failure.items():
        if run1_result.get(key) != expected:
            raise AssertionError(f"Run1 failure evidence changed: {key}")

    if not (
        diagnosis.get("outcome") == "passed"
        and diagnosis.get("process_exit_code") == 0
        and diagnosis.get("timed_out") is False
        and diagnosis.get("assertions_passed") is True
        and diagnosis.get("gpu_operations") == []
        and diagnosis.get("fallbacks_used") == []
    ):
        raise AssertionError("Dataset diagnosis run3 result changed")
    wrapper_events = [
        event for event in diagnosis.get("events", [])
        if event.get("phase") == "import_dataset_wrapper" and event.get("status") == "passed"
    ]
    contract_events = [
        event for event in diagnosis.get("events", [])
        if event.get("phase") == "contract_assertions" and event.get("status") == "passed"
    ]
    if len(wrapper_events) != 1 or len(contract_events) != 1:
        raise AssertionError("Dataset diagnosis phase evidence changed")
    wrapper_seconds = float(wrapper_events[0]["phase_seconds"])
    contract = contract_events[0]["output"]
    expected_contract = {
        "sets": ["sn500"],
        "sequence_names": ["SNGS-10004"],
        "video_rows": 1,
        "image_rows": 255,
        "frame_min": 0,
        "frame_max": 254,
    ...
    if contract != expected_contract or not (1100.0 < wrapper_seconds < 1200.0):
        raise AssertionError("Dataset diagnosis timing or 255-frame contract changed")

    config_differences = leaf_differences(run1_config, run2_config)
    allowed_config_differences = {
        "experiment_subname",
        "hydra.run.dir",
        "hydra.sweep.dir",
    }
    if config_differences != allowed_config_differences:
        raise AssertionError(f"Unexpected run2 config differences: {sorted(config_differences)}")

    run1_worker = Path(run1_manifest["source_files"]["worker"]).read_text(encoding="utf-8")
    run2_worker = Path(manifest["source_files"]["worker"]).read_text(encoding="utf-8")
    normalized_run2_worker = run2_worker.replace(
        "prerefiner_enrichment_run2_prepared",
        "prerefiner_enrichment_run1_prepared",
    )
    if normalized_run2_worker != run1_worker:
        raise AssertionError("Run2 worker differs from run1 beyond the immutable run identity")

    manifest_differences = leaf_differences(run1_manifest, manifest)
    allowed_manifest_differences = {
        "stage",
        "description",
        "references.run1_config",
        "references.run1_manifest",
        "references.run1_result",
        "references.dataset_diagnosis_run3_result",
        "source_files.launcher",
        "source_files.worker",
        "config.name",
        "config.path",
        "adapters.local_cache_dir",
        "timeouts_seconds.instantiate_dataset",
        "approval.required_environment",
        "approval.approval_scope",
        "outputs.hydra_run_dir",
        "outputs.state_archive",
        "outputs.preflight_report_dir",
    ...
    if manifest_differences != allowed_manifest_differences:
        raise AssertionError(f"Unexpected run2 manifest differences: {sorted(manifest_differences)}")
    if run1_manifest["timeouts_seconds"]["instantiate_dataset"] != 1200:
        raise AssertionError("Run1 dataset timeout evidence changed")
    if manifest["timeouts_seconds"]["instantiate_dataset"] != 1800:
        raise AssertionError("Run2 dataset timeout is not 1800 seconds")
    if not (
        manifest["timeouts_seconds"]["overall"]
        == run1_manifest["timeouts_seconds"]["overall"]
        == 28800
    ):
        raise AssertionError("Overall timeout changed")
    if 1800.0 - wrapper_seconds < 600.0:
        raise AssertionError("Run2 dataset timeout lacks the declared diagnosis-based margin")
    return {
        "run1_failure_contract": expected_run1_failure,
        "diagnosis_run3_contract": expected_contract,
        "diagnosis_wrapper_seconds": wrapper_seconds,
        "run1_dataset_timeout_seconds": 1200,
        "run2_dataset_timeout_seconds": 1800,
        "diagnosis_based_margin_seconds": 1800.0 - wrapper_seconds,
        "overall_timeout_seconds": 28800,
        "config_leaf_differences": sorted(config_differences),
        "manifest_leaf_differences": sorted(manifest_differences),
        "worker_change": "manifest_identity_only",
...
def validate_input_state_metadata(manifest: dict[str, Any]) -> dict[str, Any]:
    spec = manifest["input_state"]
    path = Path(spec["path"])
    if not path.is_file() or path.stat().st_size != spec["bytes"]:
        raise AssertionError("Input state metadata changed")
    return {
        "path": str(path),
        "bytes": path.stat().st_size,
        "sha256_pinned_not_recomputed": spec["sha256"],
        "members_pinned_not_opened": spec["members_exact"],
        "detection_columns": spec["required_detection_columns"],
        "image_columns": spec["required_image_columns"],
        "archive_contents_read": False,
...
def validate_runtime_contract(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if manifest["timeouts_seconds"]["instantiate_dataset"] != 1800:
        raise AssertionError("Run2 dataset startup timeout changed")
    if manifest["timeouts_seconds"]["overall"] != 28800:
        raise AssertionError("Run2 overall timeout changed")
    if manifest["heartbeat_seconds"] != 30:
        raise AssertionError("Heartbeat interval changed")
    approval = manifest["approval"]
    if approval["required_environment"] != "G10_PREREFINER_ENRICHMENT_RUN2_GPU_APPROVED=YES":
...
def static_preflight(manifest: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    run2_delta = validate_run2_delta(manifest)
    config = validate_config(manifest)
    frames = validate_frames(manifest["sample"])
    state = (
        validate_input_state_metadata(manifest)
        if mode == "preflight"
        else validate_input_state(manifest)
    )
...
def main() -> int:
    ...
    result = {
        "schema_version": 1, "gate": "G10-B",
        "stage": "prerefiner_enrichment_run2_preflight",
