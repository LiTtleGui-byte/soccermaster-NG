#!/usr/bin/env python3
"""Phase-observable worker for guarded G10-B TrackLab Step 1 isolated runs."""
...
def main() -> int:
    ...
    model_targets = {
        spec["target"]: name for name, spec in manifest["models"].items()
    }

    def validate_weight_specs(component: str) -> list[dict[str, Any]]:
        validated = []
        for spec in manifest["models"][component]["weight_reads"]:
            path = Path(spec["path"])
    ...
    def dataset_instantiate(config: Any, *values: Any, **kwargs: Any) -> Any:
        ...
        def diagnostic_makedirs(name: Any, *args: Any, **inner_kwargs: Any) -> None:
    ...
    def staged_instantiate(config: Any, *values: Any, **kwargs: Any) -> Any:
        ...
        if component is not None:
            ...
            weights = validate_weight_specs(component)
            ...
            return run_phase(
                phase,
                instantiate_model,
                {"target": target, "declared_weight_reads": weights},
            )
        if target == "tracklab.engine.OfflineTrackingEngine":
            ...
            def instantiate_engine() -> Any:
                ...
                adapter = manifest["adapters"].get("engine_null_callback_filter")
                if adapter is not None and adapter["enabled"]:
                    from omegaconf import OmegaConf, open_dict

                    if not adapter["copy_before_filter"]:
                        raise AssertionError("Engine callback config must be copied before filtering")
                    ...
                    if adapter.get("require_struct_before") and not struct_before:
                        raise AssertionError("Engine callbacks were not in struct mode before filtering")
                    deletion_mode = adapter.get("deletion_mode", "direct")
                    if deletion_mode == "omegaconf_open_dict":
                        with open_dict(callbacks):
                            for key in null_keys:
                                del callbacks[key]
                    elif deletion_mode == "direct":
                        for key in null_keys:
                            del callbacks[key]
                    else:
                        raise AssertionError(f"Unsupported callback deletion mode: {deletion_mode}")
                    struct_after = bool(OmegaConf.is_struct(callbacks))
                    if adapter.get("require_struct_restored") and not struct_after:
                        raise AssertionError("Engine callbacks struct mode was not restored")
                    ...
                    if any(value is None for value in callbacks.values()):
                        raise AssertionError("Engine callbacks still contain None after filtering")
                    engine_output.update(
                        {
                            "callback_config_copied": True,
                            "callback_deletion_mode": deletion_mode,
    ...
    def staged_pipeline(*values: Any, **kwargs: Any) -> Any:
        return run_phase(
            "build_pipeline",
            lambda: original_pipeline(*values, **kwargs),
    ...
    def staged_tracker_state(*values: Any, **kwargs: Any) -> Any:
        return run_phase(
            "build_tracker_state",
            lambda: original_tracker_state(*values, **kwargs),
            {"state_archive": manifest["outputs"]["state_archive"]},
        )
