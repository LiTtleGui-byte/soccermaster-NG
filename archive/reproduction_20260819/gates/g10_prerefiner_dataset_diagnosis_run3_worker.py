#!/usr/bin/env python3
"""Future CPU-only worker for staged pre-Refiner dataset diagnosis run3."""
...
def main() -> int:
    ...
    def hydra_cli_compose() -> tuple[Any, dict[str, Any]]:
        ...
        hydra = Hydra.create_main_hydra2(
            task_name="g10_prerefiner_dataset_diagnosis_run3",
        ...
        full_snapshot = inside_repo(Path(manifest["outputs"]["full_config_unresolved"]))
        dataset_snapshot = inside_repo(Path(manifest["outputs"]["dataset_config_resolved"]))
        if full_snapshot.parent != dataset_snapshot.parent:
            raise AssertionError("Run3 config snapshots must share one Hydra directory")
        full_snapshot.parent.mkdir(parents=True, exist_ok=False)
        OmegaConf.save(cfg, full_snapshot, resolve=False)
        OmegaConf.save(cfg.dataset, dataset_snapshot, resolve=True)
        output.update({
            "full_config_snapshot": str(full_snapshot),
            "full_config_resolved": False,
            "dataset_config_snapshot": str(dataset_snapshot),
            "dataset_config_resolved": True,
        })
        composed["cfg"] = cfg
        return cfg, output
