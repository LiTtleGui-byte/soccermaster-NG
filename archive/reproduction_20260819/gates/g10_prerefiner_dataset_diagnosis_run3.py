#!/usr/bin/env python3
"""Static preflight and future launcher for CPU-only pre-Refiner dataset diagnosis run3."""
...
DEFAULT_MANIFEST = REPO / "reproduction/manifests/g10_prerefiner_dataset_diagnosis_run3_sngs10004.json"
EXPECTED_IDENTITY = (1, "G10-B", "prerefiner_dataset_diagnosis_run3_prepared")
...
def worker_ast_contract(worker: Path, required_phases: list[str]) -> dict[str, Any]:
    ...
    if "OmegaConf.save(cfg, full_snapshot, resolve=False)" not in source:
        raise AssertionError("Worker lost the full unresolved Hydra snapshot")
    if "OmegaConf.save(cfg.dataset, dataset_snapshot, resolve=True)" not in source:
        raise AssertionError("Worker lost the resolved dataset-only snapshot")
    return {
...
def static_preflight(manifest: dict[str, Any], manifest_path: Path, mode: str) -> dict[str, Any]:
    ...
    run2_result = json.loads(Path(manifest["references"]["run2_result"]["path"]).read_text(encoding="utf-8"))
    if not (
        run2_result.get("outcome") == "failed"
        and run2_result.get("process_exit_code") == 1
        and run2_result.get("timed_out") is False
        and run2_result.get("assertions_passed") is False
        and run2_result.get("evaluation_started") is False
        and run2_result.get("training_started") is False
        and run2_result.get("gpu_operations") == []
        and run2_result.get("fallbacks_used") == []
    ):
        raise AssertionError("Run2 failure summary changed")
    run2_failed_events = [
        event for event in run2_result.get("events", [])
        if event.get("phase") == "hydra_cli_compose" and event.get("status") == "failed"
    ]
    if len(run2_failed_events) != 1 or "hydra.job.num" not in run2_failed_events[0].get("error", ""):
        raise AssertionError("Run2 Hydra snapshot failure contract changed")
    ...
    for key in ("full_config_unresolved", "dataset_config_resolved"):
        if outputs[key].parent != outputs["hydra_run_dir"]:
            raise AssertionError(f"Config snapshot escapes future Hydra run directory: {key}")
    ...
    return {
        ...
        "run2_failure_contract": {
            "outcome": "failed",
            "process_exit_code": 1,
            "timed_out": False,
            "failed_phase": "hydra_cli_compose",
            "missing_interpolation": "hydra.job.num",
            "dataset_wrapper_started": False,
            "gpu_operations": [],
            "fallbacks_used": [],
        },
...
def run_diagnosis(manifest: dict[str, Any], manifest_path: Path, preflight: dict[str, Any]) -> int:
    ...
    result = {
        "schema_version": 1,
        "gate": "G10-B",
        "stage": "prerefiner_dataset_diagnosis_run3",
...
def main() -> int:
    ...
    result = {
        "schema_version": 1,
        "gate": "G10-B",
        "stage": "prerefiner_dataset_diagnosis_run3_preflight",
        "outcome": "passed",
