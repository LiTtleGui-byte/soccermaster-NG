...
class FeatureExtractor:
    ...
    def extract_features(self, input_data, chunk_size=None):
        ...
        if input_data.dim() == 4:
            ...
            # 滑动窗口特征提取逻辑 (stride=1)
            # 对于每一帧，使用它作为chunk中心帧时提取的特征
            half_chunk = chunk_size // 2
            
            # 1. 为每一帧确定其对应的chunk范围
            chunks_to_process = []
            for frame_idx in range(total_frames):
                # 理想情况下，该帧应该在chunk的中心位置
                chunk_start = frame_idx - half_chunk
                
                # 边界处理
                if chunk_start < 0:
                    # 左边界：使用第一个chunk
                    chunk_start = 0
                elif chunk_start + chunk_size > total_frames:
                    # 右边界：使用最后一个可能的chunk
                    chunk_start = max(0, total_frames - chunk_size)
                
                chunk_end = min(chunk_start + chunk_size, total_frames)
                chunks_to_process.append((frame_idx, chunk_start, chunk_end))
            
            # 2. 处理所有chunk（使用缓存避免重复计算相同的chunk）
            chunk_cache = {}
            final_features = [None] * total_frames
            
            for frame_idx, chunk_start, chunk_end in chunks_to_process:
                chunk_key = (chunk_start, chunk_end)
                
                # 如果该chunk还未处理，则进行处理并缓存
                if chunk_key not in chunk_cache:
                    chunk = input_data[chunk_start:chunk_end]
                    curr_size = chunk.shape[0]
                    
                    # 如果chunk大小不足，进行padding
                    ...
                    feats = output['global_features'].squeeze(0)
                    chunk_cache[chunk_key] = feats.cpu()
                
                # 获取该帧在chunk中的位置，并提取对应特征
                pos_in_chunk = frame_idx - chunk_start
                final_features[frame_idx] = chunk_cache[chunk_key][pos_in_chunk]
            
            # 3. 返回所有帧的特征
            return torch.stack(final_features, dim=0).numpy()
...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, required=True, help='输出目录', default='/mnt/vision_user/sports/datasets/soccernet_v2_soccer_master_sliding_window_features')
