...
class SoccerPitchVisualizer:
    def __init__(self, pitch_length=105., pitch_width=68.):
        self.pitch = SoccerPitch(pitch_length, pitch_width)
        self.fig, self.ax = plt.subplots(figsize=(12, 8))
        self.ax.set_aspect('equal')
        self.ax.set_xlim(-pitch_length/2 - 5, pitch_length/2 + 5)
        self.ax.set_ylim(-pitch_width/2 - 5, pitch_width/2 + 5)
        self.ax.set_title('Soccer Pitch Visualization')
        self.ax.grid(False)
        self.ax.set_facecolor('green')
        
    def draw_line(self, start, end, color, label):
        """绘制直线并添加标签"""
        self.ax.plot([start[0], end[0]], [start[1], end[1]], 
                     color=np.array(color)/255, linewidth=2)
        # 在线的中点添加标签
        mid_x = (start[0] + end[0]) / 2
        mid_y = (start[1] + end[1]) / 2
        self.ax.text(mid_x, mid_y, label, fontsize=8, 
                     ha='center', va='center', 
                     bbox=dict(facecolor='white', alpha=0.7, edgecolor='none'))

    def draw_arc(self, center, radius, theta1, theta2, color, label):
        """绘制圆弧并添加标签"""
        ...
        mid_x = center[0] + radius * np.cos(angle)
        mid_y = center[1] + radius * np.sin(angle)
        self.ax.text(mid_x, mid_y, label, fontsize=8, 
                     ha='center', va='center', 
                     bbox=dict(facecolor='white', alpha=0.7, edgecolor='none'))
    ...
    def visualize(self):
        # 绘制所有直线
        for line_class, (start, end) in self.pitch.line_extremities.items():
            if "Circle" not in line_class:  # 排除圆弧
                color = self.pitch.palette.get(line_class, (0, 0, 0))
                self.draw_line(start, end, color, line_class)
        
        # 绘制中心圆
        center = self.pitch.center_mark
        self.draw_arc(center, SoccerPitch.CENTER_CIRCLE_RADIUS, 
                      0, 360, (0, 0, 255), "Circle central")
        
        # 绘制左侧罚球弧
        left_center = self.pitch.left_penalty_mark
        self.draw_arc(left_center, SoccerPitch.CENTER_CIRCLE_RADIUS, 
                      -50, 50, (255, 127, 0), "Circle left")
        
        # 绘制右侧罚球弧
        right_center = self.pitch.right_penalty_mark
        self.draw_arc(right_center, SoccerPitch.CENTER_CIRCLE_RADIUS, 
                      130, 230, (0, 255, 255), "Circle right")
        
        # 添加图例
        plt.legend(handles=[
            plt.Line2D([0], [0], color=np.array(color)/255, lw=2) 
            for color in self.pitch.palette.values()
        ], labels=self.pitch.palette.keys(), 
           loc='upper right', bbox_to_anchor=(1.25, 1))
        
        plt.tight_layout()
        # plt.show()
        plt.savefig('soccer_pitch.png')

# 使用示例
if __name__ == "__main__":
    visualizer = SoccerPitchVisualizer()
