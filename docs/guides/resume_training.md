...
当前研究配置位于`research/configs/`，运行产物统一写入`runs/`。下面示例中的路径均以项目根目录为基准。
...
OUTPUTS_DIR: ./runs/training/your_experiment_name
```
...
```yaml
RESUME_FROM_CHECKPOINT_DIR: ./runs/training/your_experiment_name/epoch_10
```
...
```bash
# 自动恢复
python train.py --config-path research/configs/your_config.yaml --resume-training True

# 从指定checkpoint恢复
python train.py --config-path research/configs/your_config.yaml --resume-from-checkpoint-dir ./runs/training/experiment/epoch_10
```
...
```yaml
# research/configs/my_experiment.yaml
...
运行训练：
```bash
python train.py --config-path research/configs/my_experiment.yaml
```
...
# 指定特定checkpoint
RESUME_FROM_CHECKPOINT_DIR: ./runs/training/my_experiment/epoch_5
