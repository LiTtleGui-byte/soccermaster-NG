...
SOURCE_ROOT = REPO / "research/src"
OPS_BUILD = SOURCE_ROOT / "soccermaster/models/deformable_detr/ops/build/lib.linux-x86_64-cpython-310"
CALIBRATION_SOURCE = REPO / "baseline/code/sn_calibration/src"
CONFIG_PATH = REPO / "research/configs/baseline/soccermaster_high_resolution.yaml"
DEFAULT_CONFIG_PATH = REPO / "research/configs/baseline/default.yaml"
MODEL_PATH = Path(os.environ.get(
    "SOCCERMASTER_SIGLIP2_PATH",
    "/mnt/nas/tianlin/SoccerMasterAssets/commentary_generation_epoch11_20260817/"
    "models/siglip2-large-patch16-512",
))
CHECKPOINT_DIR = Path(os.environ.get(
    "SOCCERMASTER_CHECKPOINT_DIR",
    str(REPO / "assets/checkpoints/official/soccermaster/epoch_19"),
))
DATA_VIEW = REPO / ".runtime/local_takeover/soccermaster_five_heads_sngs10004/dataloader_view"

REPORT = Path(os.environ.get(
    "SOCCERMASTER_FIVE_HEAD_REPORT_DIR",
    str(REPO / "runs/path_smoke_20260819/soccermaster/artifacts"),
))
...
def heartbeat(started: float) -> None:
    while True:
        time.sleep(30)
        print(f"[HEARTBEAT] elapsed_seconds={time.monotonic() - started:.0f}", flush=True)
...
def main() -> int:
    ...
    if REPORT.exists() or REPORT.is_symlink():
        raise FileExistsError(f"Refusing to reuse report directory: {REPORT}")
    ...
    result = {
        ...
        "ops_build": str(OPS_BUILD),
        "siglip2_initialization": str(MODEL_PATH),
