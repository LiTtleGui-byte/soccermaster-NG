...
BASE = REPO / "archive/reproduction_20260819/manifests/g10_prerefiner_enrichment_run2_sngs10004.json"
CONFIG = REPO / "research/reproduction/smokes/soccerfactory/configs/enrichment.yaml"
INPUT = REPO / ".runtime/path_smoke_20260819/soccerfactory/sngs10004_step1/states/sn-gamestate.pklz"
RUN_DIR = REPO / ".runtime/path_smoke_20260819/soccerfactory/sngs10004_enrichment"
CACHE = REPO / ".runtime/path_smoke_20260819/soccerfactory/sngs10004_enrichment_cache"
RUNTIME_MANIFEST = REPO / ".runtime/path_smoke_20260819/soccerfactory/sngs10004_enrichment_manifest.json"
REPORT = REPO / "runs/path_smoke_20260819/soccerfactory/enrichment_sngs10004"
RESULT = REPORT / "result.json"
EVENTS = REPORT / "events.jsonl"
LOG = REPORT / "run.log"
PYTHON = REPO / ".envs/SoccerMaster-repro/bin/python"
WORKER = REPO / "archive/reproduction_20260819/gates/g10_prerefiner_enrichment_run2_worker.py"
QWEN = REPO / "assets/checkpoints/official/soccerfactory/pretrained_models/jn/Qwen2.5-VL-7B-Instruct"
VENDOR = [REPO / "baseline/code/soccerfactory/tracklab", REPO / "baseline/code/soccerfactory/sn-gamestate", REPO]
...
def build_manifest() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    manifest.pop("fixed_evidence", None)
    manifest["python"] = str(PYTHON)
    manifest["source_files"]["tracklab_main"] = str(
        REPO / "baseline/code/soccerfactory/tracklab/tracklab/main.py"
    )
    manifest["source_files"]["worker"] = str(WORKER)
    manifest["source_files"]["launcher"] = str(Path(__file__).resolve())
    ...
    manifest["sample"].update({
        ...
        "frame_root": str(REPO / "assets/data/SN-GSR-2024/SoccerNetGS/sn500/SNGS-10004/img1"),
    })
    manifest["input_state"].update({"path": str(INPUT), "bytes": INPUT.stat().st_size})
    local_models = REPO / "assets/checkpoints/official/soccerfactory/pretrained_models"
    ...
    for historical_key in ("preflight_report_dir", "preflight_result", "preflight_log"):
        manifest["outputs"].pop(historical_key, None)
