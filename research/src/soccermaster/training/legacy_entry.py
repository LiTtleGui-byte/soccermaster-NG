...
from soccermaster.utils.logger import Logger, MetricsTracker, TPS, Metrics
from soccermaster.models.multi_task import MultiTaskingSigLIP
from soccermaster.training.runtime_option import runtime_option
from soccermaster.utils.misc import is_distributed, set_seed
from soccermaster.config import load_super_config, update_config, yaml_to_dict
from soccermaster.models.build import build_loss_fn, build_metrics_fn
...
def save_training_state(outputs_dir, epoch, model, optimizer, scheduler, train_states, accelerator, logger):
    ...
    temporary_epoch_dir = f"{epoch_dir}.tmp"
    checkpoint_ok = torch.ones(1, dtype=torch.int32, device=accelerator.device)

    if accelerator.is_main_process:
        try:
            if os.path.exists(epoch_dir) or os.path.exists(temporary_epoch_dir):
                raise FileExistsError(
                    f"Refusing to overwrite checkpoint or temporary evidence: "
                    f"{epoch_dir}, {temporary_epoch_dir}"
                )

            os.makedirs(temporary_epoch_dir)
            original_model = model.module if hasattr(model, 'module') else model
            original_model.save_checkpoint(temporary_epoch_dir, logger)

            optimizer_state_file = os.path.join(
                temporary_epoch_dir, "optimizer_state.pt"
            )
            torch.save(optimizer.state_dict(), optimizer_state_file)

            scheduler_state_file = os.path.join(
                temporary_epoch_dir, "scheduler_state.pt"
            )
            ...
            training_state = {
                ...
                "rng_scope": "rank0_only_not_exact_distributed_resume",
                "random_states": {
                    "torch_cpu": torch.get_rng_state().tolist(),
                    "torch_cuda": (
                        torch.cuda.get_rng_state().tolist()
            ...
            training_state_file = os.path.join(
                temporary_epoch_dir, "training_state.json"
            )
            with open(training_state_file, "w") as f:
                json.dump(training_state, f, indent=2)

            checkpoint_files = []
            for root, _, filenames in os.walk(temporary_epoch_dir):
                for filename in sorted(filenames):
                    path = os.path.join(root, filename)
                    checkpoint_files.append(
                        {
                            "path": os.path.relpath(path, temporary_epoch_dir),
            ...
            with open(
                os.path.join(temporary_epoch_dir, "checkpoint_manifest.json"),
                "w",
            ) as f:
                json.dump(manifest, f, indent=2, sort_keys=True)
            with open(os.path.join(temporary_epoch_dir, "COMPLETE"), "w") as f:
                f.write("complete\n")

            os.replace(temporary_epoch_dir, epoch_dir)
            logger.info(
                f"[TRANSACTIONAL_CHECKPOINT_COMPLETE] epoch={epoch} "
                f"global_step={train_states['global_step']} path={epoch_dir}"
            )
        except Exception as exc:
            checkpoint_ok.zero_()
            print(
                f"[TRANSACTIONAL_CHECKPOINT_FAILED] {type(exc).__name__}: {exc}",
                flush=True,
            )

    if is_distributed():
        torch.distributed.broadcast(checkpoint_ok, src=0)
    if checkpoint_ok.item() != 1:
        raise RuntimeError(
            "Rank 0 failed to save the transactional checkpoint; "
            "temporary evidence was preserved and no retry was attempted"
        )
...
def load_training_state(checkpoint_dir, ckpt_type, model, optimizer, scheduler, train_states, accelerator, logger):
    ...
    if "random_states" in training_state:
        ...
        if "torch_cpu" in random_states:
            torch.set_rng_state(
                torch.tensor(random_states["torch_cpu"], dtype=torch.uint8)
            )
            if random_states.get("torch_cuda") and torch.cuda.is_available():
                torch.cuda.set_rng_state(
                    torch.tensor(random_states["torch_cuda"], dtype=torch.uint8)
                )
        else:
            if random_states.get("python"):
                torch.set_rng_state(
                    torch.tensor(random_states["python"], dtype=torch.uint8)
                )
            if random_states.get("numpy") and hasattr(torch.random, "set_rng_state"):
                torch.random.set_rng_state(
                    torch.tensor(random_states["numpy"], dtype=torch.uint8)
                )
            if random_states.get("cuda") and torch.cuda.is_available():
                torch.cuda.set_rng_state(
                    torch.tensor(random_states["cuda"], dtype=torch.uint8)
                )
...
def train_engine(config: dict):
    ...
    if config["MODEL_ARCH"] == "multitask":
        ...
        from soccermaster.models.yolo import YOLOModel
    ...
    max_epochs_this_run = config.get("MAX_EPOCHS_THIS_RUN")
    if max_epochs_this_run is not None:
        if not isinstance(max_epochs_this_run, int) or max_epochs_this_run <= 0:
            raise ValueError("MAX_EPOCHS_THIS_RUN must be a positive integer when set")
        stop_epoch = min(
            config["EPOCHS"],
            train_states["start_epoch"] + max_epochs_this_run,
        )
        logger.info(
            f"[MAX_EPOCHS_THIS_RUN] start_epoch={train_states['start_epoch']} "
            f"stop_epoch_exclusive={stop_epoch}"
        )
    else:
        stop_epoch = config["EPOCHS"]

    for epoch in range(train_states["start_epoch"], stop_epoch):
        # Train one epoch:
        max_steps_reached = train_one_epoch(
            config=config,
            accelerator=accelerator,
            states=train_states,
            epoch=epoch,
        ...
        if max_steps_reached:
            logger.info(
                f"Stopping cleanly after MAX_OPTIMIZER_STEPS={config['MAX_OPTIMIZER_STEPS']} "
                f"at global_step={train_states['global_step']}; skipping scheduler, evaluation, and checkpoint."
            )
            break
        scheduler.step()
        if is_distributed():
        ...
        if (
            ...
            and not config.get("SKIP_EVALUATION", False)
            and (epoch + 1) % config["EVAL_PER_EPOCH"] == 0
        ):
        ...
        elif dataloader_test_dict and config.get("SKIP_EVALUATION", False):
            logger.info(
                f"[EVALUATION_SKIPPED_BY_CONFIG] epoch={epoch} "
...
def train_one_epoch(
    ...
    if max_optimizer_steps is not None:
        if not isinstance(max_optimizer_steps, int) or max_optimizer_steps <= 0:
            raise ValueError("MAX_OPTIMIZER_STEPS must be a positive integer when set")
        if states["global_step"] >= max_optimizer_steps:
            raise ValueError(
                "MAX_OPTIMIZER_STEPS must be greater than the starting global_step"
            )
    ...
    for cur_iter in range(start_iteration, max_iterations):
        ...
        if (
            ...
            and states["global_step"] >= max_optimizer_steps
        ):
            accelerator.wait_for_everyone()
            logger.info(
                f"[MAX_OPTIMIZER_STEPS_REACHED] global_step={states['global_step']} "
                f"limit={max_optimizer_steps}"
            )
            return True
