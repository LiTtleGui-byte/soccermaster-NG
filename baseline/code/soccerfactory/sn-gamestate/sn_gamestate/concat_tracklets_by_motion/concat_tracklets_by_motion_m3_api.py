...
def try_merge_segment(detections, track_to_image_ids, tracklet_segments, segment, image_height, image_width, margin=30, max_frames=50, max_dist=5):
    ...
    for i, segment_i in enumerate(tracklet_segments):
        ...
        # 获取max_frames内的所有检测点进行线性回归
        relevant_image_ids = [img_id for img_id in segment_i.image_ids if img_id >= segment.start_image_id - max_frames and img_id < segment.start_image_id]
        relevant_detections = detections_a[detections_a.image_id.astype(int).isin(relevant_image_ids)]
        
        # 如果数据点太少，无法进行有效的线性回归
        if len(relevant_detections) < 2:
            ...
            # 准备线性回归数据
            X = relevant_detections.image_id.astype(int).values.reshape(-1, 1)
            y_x = np.array([d['x_bottom_middle'] for d in relevant_detections.bbox_pitch.values])
            y_y = np.array([d['y_bottom_middle'] for d in relevant_detections.bbox_pitch.values])
            
            # 对x和y坐标分别进行线性回归
            model_x = LinearRegression().fit(X, y_x)
            model_y = LinearRegression().fit(X, y_y)
            
            # 预测segment.start_image_id处的位置
            predicted_x = model_x.predict([[segment.start_image_id]])[0]
            predicted_y = model_y.predict([[segment.start_image_id]])[0]
            predicted_position = np.array([predicted_x, predicted_y])
            
            # 计算预测位置与实际位置之间的距离
            dist = np.linalg.norm(predicted_position - fisrt_detection_b_bbox_pitch)
...
class ConcatTrackletsByMotion3(VideoLevelModule):
    input_columns = {
        "image": [],
