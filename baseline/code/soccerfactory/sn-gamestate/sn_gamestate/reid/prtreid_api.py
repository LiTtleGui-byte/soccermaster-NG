...
class PRTReId_wo_Role(DetectionLevelModule):
    collate_fn = default_collate
    input_columns = ["bbox_ltwh"]
    ...
    def process(self, batch, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        embeddings, visibility_scores, body_masks, _, _ = extract_test_embeddings(
            reid_result, self.test_embeddings
        )
...
class PRTReId_Role(DetectionLevelModule): # haolin：有用吗？好像没
    collate_fn = default_collate
    input_columns = ["bbox_ltwh"]
    ...
    def process(self, batch, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        _, _, _, _, role_cls_scores = extract_test_embeddings(
            reid_result, self.test_embeddings
        )
