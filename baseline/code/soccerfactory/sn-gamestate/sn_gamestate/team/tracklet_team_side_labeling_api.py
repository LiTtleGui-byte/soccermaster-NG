...
class TrackletTeamSideLabeling2(VideoLevelModule):
    ...
    def process(self, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        # Calculate average x coordinate for each tracklet in team A
        team_a_tracklets = team_a.groupby('track_id').apply(
            lambda x: np.nanmean([bbox["x_bottom_middle"] if isinstance(bbox, dict) else np.nan for bbox in x.bbox_pitch])
        )
        # Calculate average x coordinate for each tracklet in team B 
        team_b_tracklets = team_b.groupby('track_id').apply(
            lambda x: np.nanmean([bbox["x_bottom_middle"] if isinstance(bbox, dict) else np.nan for bbox in x.bbox_pitch])
        )

        # Calculate team averages from tracklet averages
        avg_a = np.nanmean(team_a_tracklets)
        avg_b = np.nanmean(team_b_tracklets)
