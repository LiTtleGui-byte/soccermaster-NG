...
class SoccerSequenceCachedDataset(Dataset):
    def __init__(self,
        ...
        self.all_processed_videos = {}
        self.all_permuted_gt = {}
        ...
        for vid_idx, vid in enumerate(tqdm(self.vid_id_list, desc="Processing videos")):
            ...
            # 存储处理后的数据
            self.all_processed_videos[vid_idx] = input_data
            self.all_permuted_gt[vid_idx] = permuted_gt
    ...
    def _compute_normalization_stats(self):
        ...
        with zipfile.ZipFile(os.path.join(self.pipeline_dir, self.pklz_files[0])) as zf:
            for vid in tqdm(self.vid_id_list[:min(10, len(self.vid_id_list))], desc="Computing stats"):  # 使用部分数据计算统计量
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # 从预处理的数据中获取对应的视频数据
        input_data = self.all_processed_videos[vid_idx]
        permuted_gt = self.all_permuted_gt[vid_idx]
        
        # 提取clip数据
        clip_data = {}
        for key in input_data:
            clip_data[key] = input_data[key][start_frame:end_frame].clone()
        
        # 添加GT数据
        for key in permuted_gt:
            clip_data[f'gt_{key}'] = permuted_gt[key][start_frame:end_frame].clone()
