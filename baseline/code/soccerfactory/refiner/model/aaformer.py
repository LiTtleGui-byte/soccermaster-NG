...
class SoccerTrackerTransformerAAFormer(nn.Module):
    """A variant of SoccerTrackerTransformer using global self-attention + spatial attention"""
    def __init__(self, 
                 d_model: int = 512,
                 nhead: int = 8,
                 num_layers: int = 6,
        ...
        # AAFormer风格的Transformer编码器 - 先全局注意力，再空间注意力
        # 创建多个Block层
...
class AAFormerBlock(nn.Module):
    ...
    def forward(self, x, mask, B, T, N):
        ...
        # 1. 全局自注意力 - 将所有时间和空间位置展平处理
        # 重塑为(B, T*N, d)以便对所有时空位置一起处理
        x_flat = x.reshape(B, T*N, x.shape[-1])  # (B, T*N, d)
        
        # 创建全局掩码
        global_mask = mask.reshape(B, T*N)  # (B, T*N)
        
        # 应用全局自注意力
        res_global = self.drop_path(self.global_attn(self.global_norm(x_flat), global_mask))
        x_flat = x_flat + res_global
        
        # 重塑回(B,T,N,d)
        x_global = x_flat.reshape(B, T, N, x.shape[-1])  # (B,T,N,d)
        
        # 2. 空间注意力 - 在每个时间步内进行空间自注意力
        # 重塑为(B*T, N, d)以便对每个时间步处理其空间关系
        xs = x_global.reshape(B*T, N, x.shape[-1])  # (B*T,N,d)
