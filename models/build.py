...
from models.SoccerNetGSR_Lines import build_lines_criterion, build_lines_metrics
...
def build_loss_fn(config: dict):
    ...
    for task in tasks:
        if task == "SoccerNetGSR_Detection":
            ...
            loss_fn_dict[task] = build_lines_criterion(config=config)
...
def build_metrics_fn(config: dict):
    ...
    for task in tasks:
        if task == "SoccerNetGSR_Detection":
            ...
            metrics_fn_dict[task] = build_lines_metrics(config=config)
        elif task == "SoccerNetGSR_ReID":
